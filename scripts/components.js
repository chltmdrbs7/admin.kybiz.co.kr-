...
function loadSidebar(currentPage) {
  const sidebarHTML = `
    ...
    <nav class="sidebar-nav">
      ...
      <a href="/leads" class="nav-item ${currentPage === 'leads' ? 'active' : ''}">
        <span class="nav-icon">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <a href="/board" class="nav-item ${currentPage === 'board' ? 'active' : ''}">
        <span class="nav-icon">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function handleLogout() {
  if (confirm('로그아웃 하시겠습니까?')) {
    localStorage.removeItem('ky_admin_auth');
